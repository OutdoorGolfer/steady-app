...
function Dashboard({ mode, setScreen, spendLog, pendingItems, readyItems, checkins, onLogout }) {
  ...
  return (
    <div style={{minHeight:"100vh",padding:"32px 20px 24px",maxWidth:480,margin:"0 auto"}}>
      ...
      {/* ===== SECTION 1: HERO — combined mode + focus ===== */}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
        <button onClick={()=>setScreen("decision")} style={{display:"flex",alignItems:"center",gap:14,padding:"18px 20px",background:"#0F172A",border:mode==="light"?"2px solid #FBBF2450":isSurvival?"2px solid #F8717140":"2px solid #1E293B",borderRadius:16,cursor:"pointer",width:"100%",textAlign:"left",position:"relative"}}>
          ...
          <div style={{flex:1}}>
            <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:16,fontWeight:600,color:"#F1F5F9",margin:0}}>Decision Gate</p>
            <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:12,color:"#64748B",margin:"3px 0 0"}}>Should I make this decision right now?</p>
          </div>
          <span style={{color:"#334155",fontSize:18}}>›</span>
        </button>

        <button onClick={()=>setScreen("spend")} style={{display:"flex",alignItems:"center",gap:14,padding:"18px 20px",background:"#0F172A",border:isSurvival?"2px solid #F8717140":"2px solid #1E293B",borderRadius:16,cursor:"pointer",width:"100%",textAlign:"left",position:"relative"}}>
          ...
          <div style={{flex:1}}>
            <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:16,fontWeight:600,color:"#F1F5F9",margin:0}}>Spend Check</p>
            <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:12,color:isSurvival?"#F87171":"#64748B",margin:"3px 0 0"}}>{isSurvival?"⚠️ Be extra careful today":"Thinking about buying something?"}</p>
          </div>
          <span style={{color:"#334155",fontSize:18}}>›</span>
        </button>
      </div>

      {/* ===== SECTION 4: SMALL STATS ===== */}
      {totalCheckins > 0 && (
        <div style={{display:"flex",gap:0,background:"#0F172A",border:"1px solid #1E293B",borderRadius:12,marginBottom:16,overflow:"hidden"}}>
          <div style={{flex:1,padding:"10px 12px",textAlign:"center",borderRight:"1px solid #1E293B"}}>
            ...
            <p style={{fontFamily:"'Fraunces', serif",fontSize:18,color:streak>=3?"#4ADE80":"#E2E8F0",margin:"2px 0 0"}}>{streak}d</p>
          </div>
          <div style={{flex:1,padding:"10px 12px",textAlign:"center",borderRight:"1px solid #1E293B"}}>
            ...
            <p style={{fontFamily:"'Fraunces', serif",fontSize:18,color:savedAmount>0?"#4ADE80":"#E2E8F0",margin:"2px 0 0"}}>${savedAmount.toLocaleString()}</p>
          </div>
          <div style={{flex:1,padding:"10px 12px",textAlign:"center"}}>
            ...
            <p style={{fontFamily:"'Fraunces', serif",fontSize:18,color:"#E2E8F0",margin:"2px 0 0"}}>{totalCheckins}</p>
          </div>
        </div>
      )}

      {/* ===== SECTION 5: PATTERN SNAPSHOT ===== */}
      {totalCheckins >= 3 && (
        <div style={{background:"#0F172A",border:"1px solid #1E293B",borderRadius:12,padding:"14px 16px",marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:11,color:"#64748B",margin:0,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5}}>Your pattern</p>
            <button onClick={()=>setScreen("patterns")} style={{background:"none",border:"none",color:"#6366F1",fontFamily:"'DM Sans', sans-serif",fontSize:11,cursor:"pointer",padding:0}}>See all →</button>
          </div>
          {/* Mode bars */}
          {["full","light","survival"].map(md => {
            const pct = totalCheckins > 0 ? Math.round((modeCount[md] / totalCheckins) * 100) : 0;
            const colors = { full: "#4ADE80", light: "#FBBF24", survival: "#F87171" };
            return pct > 0 ? (
              <div key={md} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                <span style={{fontSize:12,width:16}}>{MODES[md].emoji}</span>
                <div style={{flex:1,height:6,background:"#1E293B",borderRadius:3,overflow:"hidden"}}>
                  <div style={{height:"100%",width:pct+"%",background:colors[md],borderRadius:3}}/>
                </div>
                <span style={{fontFamily:"'DM Sans', sans-serif",fontSize:11,color:"#64748B",width:32,textAlign:"right"}}>{pct}%</span>
              </div>
            ) : null;
          })}
          {/* Insight line */}
          {survSkipRate !== null && fullSkipRate !== null && survSkipRate < fullSkipRate && (
            <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:12,color:"#F87171",margin:"8px 0 0",lineHeight:1.4}}>⚠️ You skip less on Survival days — watch impulse spending when you're low.</p>
          )}
          {survSkipRate !== null && fullSkipRate !== null && survSkipRate >= fullSkipRate && (
            <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:12,color:"#4ADE80",margin:"8px 0 0",lineHeight:1.4}}>💪 Your skip rate holds up on hard days. Guardrails working.</p>
          )}
        </div>
      )}

      {/* ===== SECTION 6: SECONDARY TOOLS ===== */}
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
        {[
          { key: "history", icon: "📊", title: "Decision Log", sub: spendLog.length === 0 ? "Start building your pattern" : spendLog.length + " entries — patterns building" },
          { key: "patterns", icon: "🔍", title: "My Patterns", sub: "What your data says about you" },
          { key: "transition", icon: "🔄", title: "Change Gears", sub: "Help me switch what I'm doing" },
        ].map(t => (
          <button key={t.key} onClick={()=>setScreen(t.key)} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:"#0F172A",border:"1px solid #1E293B",borderRadius:12,cursor:"pointer",width:"100%",textAlign:"left"}}>
            <span style={{fontSize:20,flexShrink:0}}>{t.icon}</span>
            <div style={{flex:1}}>
              <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:14,fontWeight:500,color:"#E2E8F0",margin:0}}>{t.title}</p>
              <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:11,color:"#64748B",margin:"2px 0 0"}}>{t.sub}</p>
            </div>
            <span style={{color:"#334155",fontSize:16}}>›</span>
          </button>
        ))}

        <button onClick={()=>setScreen("quicklog")} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"transparent",border:"1px solid #1E293B",borderRadius:12,cursor:"pointer",width:"100%",textAlign:"left"}}>
          <span style={{fontSize:16}}>⚡</span>
          <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:13,color:"#64748B",margin:0}}>Quick log a purchase I already made</p>
        </button>
      </div>

      {/* ===== SECTION 7: LOWEST PRIORITY ===== */}
      <button onClick={()=>setScreen("checkin")} style={{width:"100%",padding:"12px",background:"transparent",border:"1px solid #1E293B",borderRadius:10,color:"#64748B",fontFamily:"'DM Sans', sans-serif",fontSize:13,cursor:"pointer",marginBottom:6}}>Redo check-in →</button>
