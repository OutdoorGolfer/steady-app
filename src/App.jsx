...
function SplashScreen({ onContinue }) {
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px"}}>
      ...
      <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:17,color:"#94A3B8",textAlign:"center",maxWidth:340,lineHeight:1.7,marginBottom:12}}>
        This app checks how your brain is doing today and protects you from making bad decisions when you're running low.
      </p>
      <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:15,color:"#64748B",textAlign:"center",maxWidth:320,lineHeight:1.6,marginBottom:40}}>
        You'll answer 4 quick questions. Based on your answers, the app sets your day mode and adjusts what it recommends.
      </p>
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      <div style={{width:"100%",maxWidth:400,marginBottom:40}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8}}>
          {step > 0 ? (
            <button onClick={handleBack} style={{background:"none",border:"none",color:"#6366F1",fontFamily:"'DM Sans', sans-serif",fontSize:13,cursor:"pointer",padding:"4px 0"}}>← Back</button>
          ) : <span/>}
  ...
  if (resolved) {
    ...
    return (
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        ...
        <h2 style={{fontFamily:"'Fraunces', serif",fontSize:28,fontWeight:500,color:skipped?"#4ADE80":"#FBBF24",textAlign:"center",marginBottom:12}}>{skipped?"$"+item.amount.toLocaleString()+" saved!":"Purchased — no judgment"}</h2>
        <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:16,color:"#94A3B8",textAlign:"center",maxWidth:320,lineHeight:1.6,marginBottom:8}}>{skipped?"You waited, the urge passed, and you kept your money. That's real discipline.":"You waited, thought it through, and decided you still want it. That's a good decision."}</p>
        <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:14,color:"#64748B",textAlign:"center",marginBottom:32}}>{item.item} — ${item.amount.toLocaleString()}</p>
...
function Dashboard({ mode, setScreen, spendLog, pendingItems, readyItems }) {
  ...
  return (
    <div style={{minHeight:"100vh",padding:"32px 20px",maxWidth:480,margin:"0 auto"}}>
      <div style={{background:m.bg,border:"2px solid "+m.color+"30",borderRadius:20,padding:"28px 24px",marginBottom:28,textAlign:"center"}}>
        ...
        <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:16,color:"#94A3B8",margin:0,lineHeight:1.5}}>{m.desc}</p>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <ToolButton icon="💳" title="Spend Check" subtitle="Thinking about buying something?" onClick={()=>setScreen("spend")} warning={mode==="survival"} warningText="⚠️ Survival mode — be extra careful"/>
      ...
      <button onClick={()=>setScreen("checkin")} style={{marginTop:20,width:"100%",padding:"14px",background:"transparent",border:"1px solid #1E293B",borderRadius:12,color:"#64748B",fontFamily:"'DM Sans', sans-serif",fontSize:14,cursor:"pointer"}}>Redo check-in →</button>
    </div>
  );
}
...
function ToolButton({ icon, title, subtitle, onClick, warning, warningText }) {
  return (
    <button onClick={onClick} style={{display:"flex",alignItems:"center",gap:16,padding:"20px 22px",background:"#0F172A",border:warning?"2px solid #F8717140":"2px solid #1E293B",borderRadius:18,cursor:"pointer",transition:"all 0.2s ease",width:"100%",textAlign:"left"}}>
      ...
      <div style={{flex:1}}>
        ...
        <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:13,color:warning?"#F87171":"#64748B",margin:"4px 0 0"}}>{warning?warningText:subtitle}</p>
      </div>
  ...
  if (done) {
    ...
    return (
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        ...
        <h2 style={{fontFamily:"'Fraunces', serif",fontSize:28,fontWeight:500,color:safe?"#4ADE80":"#F87171",textAlign:"center",marginBottom:12}}>{safe?"OK to decide":"Not right now"}</h2>
        ...
        <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:16,color:"#94A3B8",textAlign:"center",maxWidth:320,lineHeight:1.6,marginBottom:32}}>{safe?"You seem to be in a reasonable headspace. Go ahead — but take it slow.":"Your brain isn't in the best place for this right now. Save it for tomorrow. The decision will still be there."}</p>
...
function SpendCheck({ mode, onBack, onWait, onBuyNow }) {
  ...
  if (result==="bought") {
    return (<div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:15,color:"#94A3B8",textAlign:"center",maxWidth:300,lineHeight:1.5,marginBottom:32}}>No judgment. It's logged so you can see patterns over time.</p>
      <button onClick={onBack} style={backBtnStyle}>← Back to dashboard</button>
    </div>);
  }
  if (step===0) {
    return (<div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      {mode==="survival"&&(<div style={{background:"#2E0A0A",border:"1px solid #F8717140",borderRadius:12,padding:"12px 16px",marginBottom:28,maxWidth:340}}><p style={{fontFamily:"'DM Sans', sans-serif",fontSize:14,color:"#F87171",margin:0,textAlign:"center"}}>🔴 Survival mode active — your brain wants to spend for relief right now. That's normal, but be careful.</p></div>)}
...
function TransitionPicker({ onBack }) {
  ...
  return (<div style={{minHeight:"100vh",padding:"32px 20px",maxWidth:480,margin:"0 auto"}}>
    ...
    <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:14,color:"#64748B",marginBottom:28}}>Pick the transition you're making right now.</p>
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
...
function SpendHistory({ spendLog, onBack }) {
  ...
  return (<div style={{minHeight:"100vh",padding:"32px 20px",maxWidth:480,margin:"0 auto"}}>
    ...
    {spendLog.length>0&&(<div style={{background:"#0F172A",border:"1px solid #1E293B",borderRadius:14,padding:"16px 20px",marginBottom:24,textAlign:"center"}}>
      <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:13,color:"#64748B",margin:"0 0 4px"}}>Wait rate</p>
      <p style={{fontFamily:"'Fraunces', serif",fontSize:28,color:"#6366F1",margin:0}}>{Math.round((saved.length/spendLog.length)*100)}%</p>
      <p style={{fontFamily:"'DM Sans', sans-serif",fontSize:12,color:"#64748B",margin:"4px 0 0"}}>{saved.length} of {spendLog.length} impulses paused</p>
    </div>)}
    {spendLog.length===0?(<p style={{fontFamily:"'DM Sans', sans-serif",fontSize:15,color:"#475569",textAlign:"center",marginTop:40}}>No entries yet. Use the Spend Check when you're tempted.</p>):(
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {[...spendLog].reverse().map((entry,i)=>(<div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",background:"#0F172A",border:"1px solid #1E293B",borderRadius:12}}>
...
export default function App() {
  ...
  const handleBuyNow=(entry)=>{setSpendLog([...spendLog,entry]);};
  const handleTimerResolve=(id,decision)=>{const item=pendingItems.find(i=>i.id===id);if(item){setSpendLog([...spendLog,{item:item.item,amount:item.amount,needWant:item.needWant,decision,date:new Date().toLocaleDateString(),waitHours:item.waitHours}]);setPendingItems(pendingItems.filter(i=>i.id!==id));}setCurrentTimerAlert(null);setScreen("dashboard");};
  ...
  return (
    <div style={{background:"#020617",minHeight:"100vh",color:"#F1F5F9"}}>
      ...
      {screen==="timer-alert"&&currentTimerAlert&&<TimerAlert item={currentTimerAlert} onResolve={handleTimerResolve}/>}
    </div>
  );
}
